...
<div class="navigation-bar" @mouseenter="onEnter" @mouseleave="onLeave">

    <a class="navigation-item" href="{{ route('home') }}" >

        <div class="navbar-icon">
            <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 490.055 490.055" xml:space="preserve" class="icon">
                <path d="M438.499,264.355c-1.2-22.4-5.8-41-14.8-39.4c-8.3,1.5-14.4,17.4-15.3,40.4c-1.7,41.3-3.6,83-3.4,124.4l0.5,50
                    ...
                    c3.3-7.1,4.9-14.4,5.2-21.7l0.1-2.7v-1.9v-3.7l0.1-7.4l0.1-14.8v-29.7C441.399,343.255,440.499,303.755,438.499,264.355z" class="icon-path"/>
                <path d="M476.599,241.355c6.6-0.6,5-7.6,0.6-15.8c-14.6-26.9-37.6-53.6-63.3-78.2c-25-24-50.1-48-75.1-72.1
                    ...
                    C465.599,239.555,473.199,241.755,476.599,241.355z" class="icon-path"/>
            </svg>
        </div>
        <span class="nav-link" v-if="wordsAreVisible">{{ __('Home') }}</span>
    </a>
    @auth
        <a class="navigation-item" href="{{ route('logout') }}" v-if="wordsAreVisible">
            <span class="nav-link">{{ __('Logout')  }}</span>
        </a>


        <a class="navigation-item" href="{{ route('search') }}" v-if="wordsAreVisible">
            <span class="nav-link">{{ __('Search') }}</span>
        </a>

    @else
        @if (Route::has('register'))
            <a class="navigation-item" href="{{ route('register') }}" v-if="wordsAreVisible">
                <span class="nav-link">{{ __('Register') }}</span>
            </a>
        @endif

        @if (Route::has('login'))
            <a class="navigation-item" href="{{ route('login') }}" v-if="wordsAreVisible">
...
<script>
    new Vue({
        el: ".navigation-bar",
        data: {
            wordsAreVisible: false,
        },

        methods: {
            onEnter() {
                this.wordsAreVisible = true;
            },

            onLeave() {
                this.wordsAreVisible = false;
            }
        }
    });
</script>
